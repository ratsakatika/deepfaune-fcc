...
re-reads the master and keeps its columns (plus each image's own second and
third best species, `top2_image`/`top3_image` with scores beside
`score_image`), writing only the rows of surviving events with the
sequence-level verdict columns refreshed to the re-derived result. A Help button at the top opens a built-in guide to the
